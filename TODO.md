...
- [ ] Keyboard shortcuts: all 6 defined shortcuts work correctly when app is the active window — code-complete and unit-verified (matcher both platforms, hook dispatches the correct action per phase + `preventDefault`, panel lists every shortcut, search focus reaches the DOM); macOS ⌘H reclaimed via the native-menu override (`cargo check` clean). Awaiting the manual active-window click-through on macOS (and a Windows pass), consistent with the other cross-platform DoD lines.
