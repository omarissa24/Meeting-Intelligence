...
- [ ] **US-13 — Data is private and secure**
  ...
  - [ ] Postgres and S3 encrypted at rest (AES-256) — Postgres half satisfied: production DB is Neon, which encrypts all data at rest (AES-256) by default. S3 half: `put_object` (async + worker paths) sets `ServerSideEncryption="AES256"` (`storage/s3.py`); full tick pending object-storage provisioning (Tigris/R2) so that path runs in prod.
